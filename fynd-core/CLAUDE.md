...
| `propamm_fallback/`   | `fallback_amount_out` computes the amount out a route delivers when its `propammrouter:` legs fall back to Uniswap V3; the encoder derives `min_amount_out` from it. `FallbackFees` mirrors the router's `resolvedFee`; `FallbackPoolIndex` indexes `uniswap_v3` components by pair and fee tier |
