...
//! in the swap data. `select_fallback` picks it: the candidate among `FALLBACK_VENUE_SYSTEMS`
//! with the best simulated amount out for the leg's `amount_in`, skipping any the request
//! excludes. A pAMM leg with no selectable candidate rejects the route.
//!
...
use crate::{
    ...
    types::{ComponentId, FallbackLeg, Route, RouteExclusionFilter, Swap},
};
...
/// family).
///
/// Gate `stamp_fallbacks` and `fallback_amount_out` behind this: it lets a route without a pAMM
/// leg cost neither a market read lock nor a replay.
pub fn has_pamm_leg(route: &Route) -> bool {
    route.swaps().iter().any(is_pamm_leg)
}

/// Whether `swap` is a leg the `TychoFallbackRouter` executes.
fn is_pamm_leg(swap: &Swap) -> bool {
    swap.protocol()
...
///
/// A `propammfallback:` leg only reaches the chain through its fallback, so a pAMM whose pair has
/// no candidate in this market can never produce a quotable route. Returns `false` for any other
/// protocol system, and for a pAMM that does not name exactly two tokens: the fallback resolves
/// per leg from the swap's own pair, so a wider component is left to `select_fallback`.
pub fn must_withhold_pamm(component: &ProtocolComponent, index: &FallbackPoolIndex) -> bool {
...
/// Stamps a fallback on every pAMM leg of `route`, leaving every other leg untouched.
///
/// Each `propammfallback:` swap gets the `FallbackLeg` that `select_fallback` picks for it, so the
/// route can then be priced by `fallback_amount_out` and encoded with its fallbacks.
///
/// # Errors
///
/// The first pAMM leg that gets no fallback stops the pass; see `FallbackSelectionError`.
pub fn stamp_fallbacks(
    route: &mut Route,
    market: &MarketDataView<'_>,
    index: &FallbackPoolIndex,
    filter: &RouteExclusionFilter,
) -> Result<(), FallbackSelectionError> {
...
///
/// The candidate for the leg's pair with the best `get_amount_out` for the leg's `amount_in`,
...
/// # Errors
///
/// `NoFallbackPool` when the pair has no candidate in `index`; `AllExcluded` when it has
/// candidates but `filter` rules out every one; `NotPriceable` when no remaining candidate could
/// be simulated.
pub fn select_fallback(
    ...
    index: &FallbackPoolIndex,
    filter: &RouteExclusionFilter,
) -> Result<FallbackLeg, FallbackSelectionError> {
    let candidates = index.candidates_for(swap.token_in(), swap.token_out());
    // `NoFallbackPool` when `candidates` is empty. Drop each candidate `filter.excluded_pools()`
    // names or whose protocol system matches an entry of `filter.excluded_protocols()`
    // (`feed::component_filter::protocol_matches`); `AllExcluded` when none is left. Otherwise
    // read each remaining candidate's component and state from `market`, simulate
    // `get_amount_out(swap.amount_in())`, and return a `FallbackLeg` built from the candidate
    // paying the most; `NotPriceable` when every simulation fails.
    todo!(
        "pick the best of {} candidates for pAMM leg {} on state {:?}, minus {} excluded pools",
        candidates.len(),
        swap.component_id(),
        market.state_label(),
        filter.excluded_pools().len()
    )
}
...
pub enum FallbackSelectionError {
    ...
    /// The market holds candidates for the pair, but the request excludes every one. Drop the
    /// route: the request's filter reaches the pool the leg settles through, not only the pAMM.
    AllExcluded {
        /// The pAMM leg whose candidates are all excluded.
        component_id: ComponentId,
        /// How many candidates the request ruled out.
        excluded: usize,
    },
    /// Candidates remain, but none could be simulated for the leg's `amount_in`. Drop the route.
    NotPriceable {
...
/// `stamp_fallbacks` provides. Uses `replay_route`, so split fractions and shared-pool depletion
...
impl FallbackPoolIndex {
    ...
    fn insert(&mut self, market: &MarketDataView<'_>, component_id: ComponentId) {
        ...
        for (position, token_a) in component.tokens.iter().enumerate() {
            for token_b in &component.tokens[position + 1..] {
...
fn qualifies(component: &ProtocolComponent) -> bool {
    if !FALLBACK_VENUE_SYSTEMS.contains(&component.protocol_system.as_str()) ||
...
mod tests {
    ...
    fn test_select_fallback_picks_best_amount_out() {
        let market = market_with_fallback_pools();
        let view = market
            .try_read_blocking()
            .expect("uncontended");
    ...
    /// A candidate the request excludes, by pool or by protocol, is never chosen, and a request
    /// that excludes every candidate gets no fallback at all.
    #[test]
    #[ignore = "scaffold: select_fallback is not implemented"]
    fn test_select_fallback_skips_excluded_candidates() {
        let market = market_with_fallback_pools();
        let view = market
            .try_read_blocking()
            .expect("uncontended");
        ...
        let by_pool =
            RouteExclusionFilter::default().with_excluded_pools([BETTER_POOL.to_string()]);
        let fallback = select_fallback(&pamm_swap(), &view, &index, &by_pool).expect("one left");
        assert_eq!(fallback.component_id(), WORSE_POOL);

        let by_protocol =
            RouteExclusionFilter::default().with_excluded_protocols(["uniswap_v2".to_string()]);
        let fallback =
            select_fallback(&pamm_swap(), &view, &index, &by_protocol).expect("one left");
        assert_eq!(fallback.component_id(), WORSE_POOL);

        let all = RouteExclusionFilter::default().with_excluded_protocols(["uniswap:".to_string()]);
        let error = select_fallback(&pamm_swap(), &view, &index, &all).expect_err("none left");
        assert_eq!(
            error,
    ...
    #[test]
    #[ignore = "scaffold: select_fallback is not implemented"]
    fn test_stamp_fallbacks_only_touches_pamm_legs() {
        let market = market_with_fallback_pools();
        let view = market
            .try_read_blocking()
            .expect("uncontended");
        ...
        stamp_fallbacks(&mut route, &view, &index, &RouteExclusionFilter::default())
            .expect("candidates exist");
    ...
    #[test]
    #[ignore = "scaffold: select_fallback and fallback_amount_out are not implemented"]
