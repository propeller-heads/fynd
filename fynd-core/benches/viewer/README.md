...
| `<run>/run.json` | header facts: orders, baseline, configs, gas price, timeout |
...
finished and its orders, pairs, configs, concurrency, timeout, gas price and dataset. `index.json`
embeds each run's whole `run.json`, so the table costs no extra reads. Filter by name, config or
dataset; `↑`/`↓` move, `enter` opens, `esc` closes.
