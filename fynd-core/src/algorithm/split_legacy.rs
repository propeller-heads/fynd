//! Legacy exhaustive split-routing algorithm (pre-hardening baseline).
//!
//! Kept as an offline benchmark/visualization baseline only (not registered for production). The
//! production `split` is the portfolio router in [`split_exp`](super::split_exp).
//!
...
pub struct SplitLegacyAlgorithm {
    min_hops: usize,
    max_hops: usize,
    timeout: Duration,
...
impl SplitLegacyAlgorithm {
    /// Creates a new `SplitLegacyAlgorithm` from an [`AlgorithmConfig`].
...
impl Algorithm for SplitLegacyAlgorithm {
