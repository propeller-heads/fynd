...
| `route_filter` | `null` | Liquidity to exclude from every route: `exclude_pools` (component ids), `exclude_protocols` (exact protocol system names, e.g. `uniswap_v2`, or family prefixes, e.g. `propammfallback:`) and `exclude_tokens` (tokens no route may pass through; the order's own two are always allowed). A name the server holds no pool for excludes nothing. |
