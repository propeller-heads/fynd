...
| `derived/`            | Two `ComputationManager` instances share one store and event channel: per block, `SpotPriceComputation` then `ComponentDepthComputation`; `TokenGasPriceComputation` runs in its own throttled loop (`run_throttled`, `FyndBuilder::token_price_interval`) since its sell legs are too slow for the chain. `ReadinessTracker` gates workers until data is fresh |
