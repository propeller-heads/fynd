...
pub enum EncodingError {
    ...
    #[error(
        "checked_amount is zero after applying slippage \
         (amount_out too small for slippage_bps={slippage_bps}). Minimum checked amount is one."
    )]
...
pub struct SwapEncoder {
    /// Encoder used for [TransferType::TransferFrom] orders.
    transfer_from_encoder: Box<dyn TychoEncoder>,
    /// Encoder used for [TransferType::None] orders.
    none_encoder: Box<dyn TychoEncoder>,
    /// Shared reference to market data.
...
impl SwapEncoder {
    ...
    /// # Errors
    ///
    /// Returns an error if the default swap encoders cannot be loaded
    /// or the encoder cannot be built.
    pub fn new(chain: Chain, market_data: SharedMarketDataRef) -> Result<Self, EncodingError> {
        let build =
            |transfer_type: UserTransferType| -> Result<Box<dyn TychoEncoder>, EncodingError> {
                ...
                TychoRouterEncoderBuilder::new()
                    ...
                    .build()
                    .map_err(|e| {
                        EncodingError::EncodingFailed(format!("failed to build encoder: {e}"))
                    })
            };
    ...
    /// # Errors
    ///
    /// Returns the first encoding error encountered. Orders encoded
    /// before the failure retain their encoded data.
    pub async fn encode_solution(
        ...
        for (order_solution, order) in solution.orders.iter_mut().zip(orders) {
            ...
            order_solution.encoding = Some(
                self.encode_order(order, order_solution, options)
                    .await?,
            );
        }
        Ok(solution)
    ...
    ///
    /// Multiple pools are encoded sequentially, and the TychoRouter should be called via
    /// sequentialSwap function.
    ///
    /// Returns an error if the order has no route, encoding fails
    /// (missing component, zero checked amount, encoder failure).
    async fn encode_order(
        ...
        let route = solution
            .route
            .as_ref()
            .ok_or(EncodingError::EmptyRoute)?;

        if route.swaps.is_empty() {
            return Err(EncodingError::EmptyRoute);
        }

        // Convert fynd swaps to tycho-execution swaps
        ...
        for swap in &route.swaps {
            let component = market_data
                ...
                .ok_or_else(|| EncodingError::MissingComponent { id: swap.component_id.clone() })?;
        ...
        // Compute checked amounts using slippage provided in the encoding options
        let checked_amount = calculate_min_amount_out(&solution.amount_out, options.slippage_bps);
        if checked_amount.is_zero() && !solution.amount_out.is_zero() {
            return Err(EncodingError::ZeroCheckedAmount { slippage_bps: options.slippage_bps });
        }
        ...
        let encoded_solution = encoder
            ...
            .map_err(|e| EncodingError::EncodingFailed(format!("encode_solutions failed: {e}")))?
            .into_iter()
            .next()
            .ok_or_else(|| {
                EncodingError::EncodingFailed("encoder returned empty result".to_string())
            })?;
...
mod tests {
    ...
    use crate::{
        ...
        types::solution::{BlockInfo, Order, OrderSide, OrderSolution, Route, Swap as FyndSwap},
    };

    // Well-known Ethereum addresses for Uniswap V2 DAI/USDC pool
    fn usdc_bytes() -> [u8; 20] {
    ...
    fn make_address(bytes: [u8; 20]) -> tycho_simulation::tycho_common::models::Address {
    ...
    fn make_order(token_in: [u8; 20], token_out: [u8; 20], amount: u64) -> Order {
        Order {
    ...
    fn test_calculate_min_amount_out() {
        let test_cases = vec![
            // (amount, slippage_bps, expected_result, description)
        ...
        for (amount, slippage_bps, expected, description) in test_cases {
            ...
            assert_eq!(result, BigUint::from(expected), "failed for: {}", description);
        }
    }
    ...
    async fn encode_order_univ2_produces_calldata() {
        let component = create_univ2_component(POOL_DAI_USDC, dai_bytes(), usdc_bytes());
        let (encoder, _market_data) = create_encoder_with_component(component);
        ...
        let opts = EncodingOptions { slippage_bps: 50, transfer_type: TransferType::default() };

        let result = encoder
            .encode_order(&order, &solution, &opts)
            .await;
        assert!(result.is_ok(), "encoding failed: {:?}", result.err());

        let encoded = result.expect("already checked");
        assert!(!encoded.encoded_calldata.is_empty(), "calldata should contain actual data");
    ...
    async fn encode_order_returns_error_for_missing_component() {
        // Create encoder with empty market data
        let market_data = Arc::new(RwLock::new(SharedMarketData::new()));
        let encoder = SwapEncoder::new(Chain::Ethereum, Arc::clone(&market_data))
            .expect("failed to create encoder");
        ...
        let result = encoder
            .encode_order(&order, &solution, &opts)
            .await;
        assert!(matches!(result, Err(EncodingError::MissingComponent { .. })));
    }
    ...
    async fn encode_order_zero_checked_amount_returns_error() {
        ...
        let order = make_order(usdc_bytes(), dai_bytes(), 100_000_000);
        // Very small amount_out: 1 wei with 100% slippage => 0
        let mut solution =
            make_successful_solution(POOL_DAI_USDC, usdc_bytes(), dai_bytes(), 100_000_000, 1);
        solution.amount_out = BigUint::from(1u64);
        let opts = EncodingOptions { slippage_bps: 10_000, transfer_type: TransferType::default() };

        let result = encoder
            .encode_order(&order, &solution, &opts)
            .await;
        assert!(matches!(result, Err(EncodingError::ZeroCheckedAmount { .. })));
    }
    ...
    async fn encode_order_with_explicit_receiver() {
        ...
        let result = encoder
            .encode_order(&order, &solution, &opts)
            .await;
        assert!(result.is_ok(), "encoding with explicit receiver should work");
    }
    ...
    async fn encode_solution_encodes_successful_orders() {
        ...
        let mut solution = crate::types::solution::Solution {
        ...
        assert!(solution.orders[0].encoding.is_some(), "successful order should be encoded");
    }
    ...
    async fn encode_solution_skips_failed_orders() {
        ...
        let solution = encoder
            .encode_solution(&orders, solution, &opts)
            .await
            .expect("encoding should succeed (no successful orders to encode)");

        assert!(solution.orders[0].encoding.is_none(), "failed order should not be encoded");
    }
    ...
    async fn encode_solution_returns_error_for_missing_component() {
        ...
        assert!(matches!(solution, Err(EncodingError::MissingComponent { .. })));
    }
}
