...
impl TychoFeed {
    ...
    /// Like [`run_with_pending`](Self::run_with_pending) but additionally gates each block
    /// behind a [`BlockStepController`], delivered via `controller_tx` once the stream is
    /// built. Combines the pending-indexer and step-controller run modes for deterministic
    /// pending-block harnesses.
    ///
    ...
    pub(crate) async fn run_with_pending_and_step_controller(
        ...
        if controller_tx.send(Ok(controller)).is_err() {
            tracing::warn!(
                "BlockStepController receiver dropped before send; stream runs ungated"
            );
        }
