...
5. Triggers the `ComputationManager` → runs spot prices, token gas prices, then component (pool) depths → broadcasts `DerivedDataEvent` → workers update edge weights.
