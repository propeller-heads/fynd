...
| `worker_pool/`        | `WorkerPool` manages dedicated OS threads. `SolverWorker` runs a prioritized select loop (shutdown > market events > derived events > tasks). `TaskQueue` is `async_channel`-based. Two rules keep components out of one worker's graph: `should_drop_component` (a free function here) for exclusive components in a `PublicOnly` pool and for the pool's `exclude_protocols`, and `propamm_fallback::manager::PammManager` for pAMMs the market does not back. The first holds for the worker's life; the pAMM rule expires, so `PammManager::apply_pamm_admission` re-decides it per market event, in both directions, on `propamm_admissions_total{outcome=admitted|withheld|evicted,pool}`. `price_impact` computes every quote's `price_impact_bps` after the solve, from each swap's own spot price, walking a split route as the token-flow graph it is |
