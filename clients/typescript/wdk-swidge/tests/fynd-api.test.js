...
describe('hosted Fynd wire boundary', () => {
  it('sends one exact-input order with raw authorization, decimal string slippage and redirects disabled', async () => {
    fetchMock.mockResolvedValue(Response.json({ orders: [encodedOrder()] }))
  ...
  it('keeps unavailable gas prices unavailable on encoded quotes', async () => {
    fetchMock.mockResolvedValue(Response.json({ orders: [encodedOrder({ gas_price: null })] }))
  ...
  it.each(['no_route_found', 'insufficient_liquidity', 'timeout', 'not_ready', 'price_check_failed', 'encoding_failed'])(
    'rejects HTTP-success semantic status %s without retrying', async (status) => {
      fetchMock.mockResolvedValue(Response.json({ orders: [encodedOrder({ status, transaction: null })] }))
  ...
  it.each([
    ...
    fetchMock.mockResolvedValue(Response.json({ orders: [encodedOrder(overrides)] }))
  ...
  it.each([[], [encodedOrder(), encodedOrder()], undefined])('requires exactly one response order', async (orders) => {
    fetchMock.mockResolvedValue(Response.json({ orders }))
...
describe('HTTP failures and configuration', () => {
  ...
  it.each([
    ...
    { baseUrl: 'file:///tmp/example' }, { baseUrl: 'invalid' },
  ...
  it('allows a local proxy without an API key and preserves its base path', async () => {
    fetchMock.mockResolvedValue(Response.json({ chain_id: 8453, router_address: ROUTER, version: '0.107.1' }))
    const proxy = new FyndApi({ baseUrl: 'http://localhost:3000/fynd/', chain: 'base' })
    await expect(proxy.info()).resolves.toEqual({ chainId: 8453, routerAddress: ROUTER })
  ...
  it('preserves a quote-only chain without claiming an executable router', async () => {
    fetchMock.mockResolvedValue(Response.json({ chain_id: 1, router_address: null, version: '0.107.1' }))
...
describe('complete token discovery', () => {
  it('paginates past 1000 using raw page offsets and filters taxed or nonstandard tokens', async () => {
    ...
    first[0].quality = 99
    first[1].tax = 20
    fetchMock.mockResolvedValueOnce(Response.json({ tokens: first, total: 1002, block: 123 }))
      .mockResolvedValueOnce(Response.json({ tokens: [token(1001), token(1002)], total: 1002, block: 123 }))
  ...
  it.each([
    ...
    fetchMock.mockResolvedValueOnce(Response.json({ tokens: [token(1)], total: 2, block: 123 }))
  ...
  it('accepts a complete empty graph', async () => {
    fetchMock.mockResolvedValue(Response.json({ tokens: [], total: 0, block: 123 }))
  ...
  it('uses one deadline for the full pagination operation', async () => {
    ...
    fetchMock.mockImplementationOnce(async () => {
      vi.setSystemTime(40)
      return Response.json({ tokens: [token(1)], total: 2, block: 123 })
    }).mockImplementationOnce(async () => {
      vi.setSystemTime(101)
      return Response.json({ tokens: [token(2)], total: 2, block: 123 })
    })
