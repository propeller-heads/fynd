...
| `worker_pool/`        | `WorkerPool` manages dedicated OS threads. `SolverWorker` runs a prioritized select loop (shutdown > readiness deadline > market events > derived events > readiness transitions > solve tasks). `TaskQueue` is `async_channel`-based |
