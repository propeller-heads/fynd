...
| `worker_pool_router/` | `WorkerPoolRouter` fans out orders to all pools, ranks candidates by `amount_out_net_gas` descending; price guard (if enabled) validates in rank order; optionally encodes. `solve`/`encode_quote` split (ENG-6235); `encode_cached` re-encodes a cached solve for the quote cache |
| `cache/`              | `QuoteCache` (ENG-6236): in-memory cache of pre-encode solves keyed by a `KeyNormalizer` (token pair, amount, side, sender, receiver — never `order.id`). Sliding TTL, per-identity + global LRU caps, block staleness cutoff. Off unless `--enable-quote-cache`; hooked in the `fynd-rpc` quote handler |
