...
impl WorkerPoolRouter {
    ...
    pub async fn quote(
        ...
        // Attribution runs here, not at ranking: the price guard can pick a lower-ranked
        // fallback, the exclusive overlay replaces the head of the list, and a failed encoding
        // returns no quote at all. Only the quotes below are the ones the caller receives.
