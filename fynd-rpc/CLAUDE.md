...
|---|---|---|
| `POST /v1/quote` | `handlers::quote` | Submit orders, receive optimal routes. The `x-exclusive-access: true` request header (set by the authenticating proxy, never by the caller) allocates exclusive-access worker pools to the request; any other value or none restricts it to public pools. The `x-disable-slippage-taking: true` request header (same proxy-only trust model) makes the encoder attach server-signed zero-fee `ClientFeeParams` so the FeeCalculator applies the signer's positive-slippage exemption. Both headers are only meaningful when the server is unreachable except through that proxy. Composed of the public `validate_quote_request` / `disable_slippage_taking::apply` / `ReplayRequest::capture` / `log_quote_outcome` helpers, for embedders writing their own variant |
