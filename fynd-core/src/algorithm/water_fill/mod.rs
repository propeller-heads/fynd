...
fn can_extend_path<W>(
    ...
    cfg.search
        .allows_token(next_addr, (cfg.source_token, &graph[target]))
}
...
mod tests {
    ...
    #[rstest::rstest]
    #[case::default(false)]
    #[case::excluded_endpoints(true)]
    #[tokio::test]
    async fn test_discovery_finds_and_ranks_parallel_components(#[case] exclude_endpoints: bool) {
        ...
        let exclusions = if exclude_endpoints {
            RouteExclusions::default().with_tokens([link.address.clone(), weth.address.clone()])
        } else {
