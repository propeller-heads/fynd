...
| `--tycho-latency-buffer-secs`      | `TYCHO_LATENCY_BUFFER_SECS` | `5` on Robinhood; _(Tycho chain default)_ elsewhere | Extra time beyond the chain's block time to wait for delayed Tycho protocol updates. An explicit value overrides the chain-specific default. |
...
| `--enable-simulation`                        | —        | `false`      | Enable simulation of encoded quotes against the latest block.                                                                                                           |
