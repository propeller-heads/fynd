...
| `worker_pool/`        | `WorkerPool` manages dedicated OS threads. `SolverWorker` runs a prioritized select loop (shutdown > market events > derived events > tasks). `TaskQueue` is `async_channel`-based. `SolverWorker::drops_component` decides what stays out of one worker's graph: exclusive components in a `PublicOnly` pool, plus every protocol system in the pool's `exclude_protocols`. `price_impact` computes every quote's `price_impact_bps` after the solve, from each swap's own spot price, walking a split route as the token-flow graph it is |
