...
| `worker_pool/`        | `WorkerPool` owns and supervises dedicated OS threads. The first unexpected worker exit marks the pool unhealthy, stops sibling workers, and propagates a fatal runtime error. `SolverWorker` runs a prioritized select loop (shutdown > market events > derived events > tasks). `TaskQueue` is `async_channel`-based |
