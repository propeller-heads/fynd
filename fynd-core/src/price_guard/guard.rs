...
mod tests {
    ...
    fn test_all_providers_error(#[case] allow_on_error: bool, #[case] should_pass: bool) {
        let config =
            PriceGuardConfig::default().with_enabled(true).with_allow_on_provider_error(allow_on_error);
    ...
    fn test_one_pass_one_fail() {
        ...
        let config = PriceGuardConfig::default().with_enabled(true).with_lower_tolerance_bps(300);
    ...
    fn test_all_price_not_found(#[case] allow: bool, #[case] result_status: QuoteStatus) {
        let config =
            PriceGuardConfig::default().with_enabled(true).with_allow_on_token_price_not_found(allow);
        let guard =
