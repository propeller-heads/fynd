...
| `worker_pool/`        | `WorkerPool` manages dedicated OS threads. `SolverWorker` runs a prioritized select loop (shutdown > market events > derived events > tasks). `TaskQueue` is `async_channel`-based. `SolverWorker::drops_component_from_graph` decides what stays out of one worker's graph: exclusive components in a `PublicOnly` pool, every protocol system in the pool's `exclude_protocols`, and every pAMM this market cannot back (`lacks_fallback_pool`). The first two hold for the life of the worker; the pAMM rule expires, so `apply_pamm_admission` re-decides it on every market event — withholding one that arrives unbacked, and evicting one whose fallback pool has left |
