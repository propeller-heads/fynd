...
| `worker_pool/`        | `WorkerPool` manages dedicated OS threads. `SolverWorker` runs a prioritized select loop (shutdown > market events > derived events > tasks). `TaskQueue` is `async_channel`-based. `SolverWorker::drops_component_or_unbacked_pamm` decides what stays out of one worker's graph: exclusive components in a `PublicOnly` pool, every protocol system in the pool's `exclude_protocols`, and every unbacked pAMM (`is_unbacked_pamm`). The first two hold for the worker's life; the pAMM rule expires, so `apply_pamm_admission` re-decides it on every market event, in both directions, and counts each decision on `propamm_admissions_total{outcome=admitted|dropped|evicted,pool}` |
