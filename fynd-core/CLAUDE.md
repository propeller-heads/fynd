...
| `simulation/`         | Encoded-quote simulation, switched on per deployment with `--enable-simulation`. `token_layout` discovers a token's balance and allowance slots by tracing the storage its own view functions read, so a non-standard layout (USDT, stETH shares) can still be funded; `simulator` runs the quote under `eth_simulateV1` with those overrides; `revert` names the error a reverting call carries by replaying it under the call tracer. `erc20_slots` keeps the older sentinel-probe helpers, which the tool crates use. `deviation` is ungated: the quote log reports the same figure the metric does |
