...
/// themselves.
fn exclusive_access_from_headers(headers: &HeaderMap) -> ExclusiveAccess {
    let granted = headers
...
/// Kept separate from `tests` below because that module imports `actix_web::test`, which shadows
/// the `#[test]` attribute `rstest` generates.
#[cfg(test)]
mod exclusive_access_tests {
    use actix_web::http::header::{HeaderMap, HeaderName, HeaderValue};
    use fynd_core::ExclusiveAccess;
    use rstest::rstest;

    use super::{exclusive_access_from_headers, EXCLUSIVE_ACCESS_HEADER};
    ...
    fn test_exclusive_access_from_headers(
        #[case] header_value: Option<&str>,
        #[case] expected: ExclusiveAccess,
    ) {
        ...
        assert_eq!(exclusive_access_from_headers(&headers), expected);
    }
}
...
pub(crate) async fn quote(
    ...
    let exclusive_access = exclusive_access_from_headers(http_request.headers());
    ...
    let result = state
        .worker_router()
        .quote(core_request, exclusive_access)
        .await;
