...
| `error.rs` | `ApiError` type with HTTP status code mapping. `RequestValidationError` is the rule a request broke before solving (no orders, an order failing `Order::validate`, encoding options failing `EncodingOptions::validate`); `ApiError::InvalidRequest` answers 400 with the rule's own code (`NO_ORDERS`, `SAME_TOKENS`, `ZERO_AMOUNT`, `INVALID_SLIPPAGE`, `CLIENT_FEE_TOO_HIGH`), where `BadRequest` keeps `BAD_REQUEST` for a body that does not parse |
